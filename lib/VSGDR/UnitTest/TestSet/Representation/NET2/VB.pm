package VSGDR::UnitTest::TestSet::Representation::NET2::VB;
...
use parent qw(VSGDR::UnitTest::TestSet::Representation::NET2) ;
...
use VSGDR::UnitTest::TestSet::Representation::NET2;
...
#VSGDR::UnitTest::TestSet::Dump(@_) ;                                                                         
        ...
        testdata:       /Dim testActions As SqlDatabaseTestActions = Me\./ testdata_name[ $arg[0] ] 

        testmethod:     /\<TestMethod\(\)\>\s+Public\s+Sub/ testmethodname /\(\)/  testdata[ $item[2][1] ] /.*?(?=(?:\<TestMethod\(\)\>)|(?:End Class))/s 
...
my $grammar_pass2 = q{
        ...
        testaction:             /Dim/ m{\w++}sx /As/ /Microsoft\.Data\.Tools\.Schema\.Sql\.UnitTesting\.SqlDatabaseTestAction/
        ...
        testmethod:     /\<TestMethod\(\)\>\s+Public\s+Sub/ testmethodname /\(\)/  testdata[ $item[2][1] ] /.*?(?=(?:\<TestMethod\(\)\>)|(?:#Region "Designer support code"))/s 


        condition_type: m{\w++}sx     
        condition_name: m{\w++}sx     
        condition:      /Dim/ condition_name /As/ /Microsoft\.Data\.Tools\.Schema\.Sql\.UnitTesting\.Conditions\./ condition_type   
...
#    $self->{PARSER} = \&VSGDR::UnitTest::TestSet::Representation::NET2::VB ;
...
sub Tests {
    ...
    foreach my $test ( @tests) {
        ...
        $res .= "${p2}Dim testActions As SqlDatabaseTestActions = Me.".$test->testActionDataName()."\n";
...
#print Dumper ${arg2} ;
    ...
    $res .= "${p2}System.Diagnostics.Trace.WriteLineIf(testActions.".${arg2}." IsNot Nothing, \"${arg4}\")\n";
    $res .= "${p2}Dim ".${arg3}."() As SqlExecutionResult = TestService.Execute(Me.".${arg5}.", Me.PrivilegedContext, testActions.".${arg2}.")\n";
...
sub Footer {
    ...
    foreach my $test (@tests) {
        $res .= "${p2}Private " . $test->testActionDataName() . " As SqlDatabaseTestActions\n";
    }
...
### Strip Methods
    while ( $code =~ m{ \<TestMethod \(\) \> \s* _ \s+ Public \s+ Sub \s+ (?<testmethodname>\w+) \s* \(\) \s*
                        Dim \s testActions \s As \s SqlDatabaseTestActions \s = \s Me \. (?<testdata_name>\w+) 
...
<ROOT>
    ...
    Public Sub <@MethodName@>()
        Dim testActions As SqlDatabaseTestActions = Me.<@MethodName@>Data
